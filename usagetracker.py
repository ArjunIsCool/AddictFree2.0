...
def selectFile():
    ...
    if(file_path[-1:-4:-1] == "exe"):
        ...
        if file_path in allAppsLocations:
            prAlreadyError = tk.messagebox.showwarning("ERROR",'Program was found in list already!')
        else:
        ...
        print("Program is of incorrect format or is not an .exe")
...
def check_blocking_tasks(prInfo):
    ...
    try:
        ...
        if (totalTimeSpent > timeLimit*60*60):
            ...
            print(totalTimeSpent,'ha???')
            print(totalTimeSpent,'ha???')
            print((timeLimit*60 - notifyPrior*60),'hahshah')
            if totalTimeSpent > (timeLimit*60 - notifyPrior*60) and pName not in notifsShown:
...
def buildMenu():
    ...
    currentMenu = 'mainMenu'
    title = tk.Label(appWindow,text='App Usage Tracker and Blocker',bg='lightblue',font=('Arial',50))
    title.grid(row=0,column=1)


    button1 = tk.Button(appWindow,text='Track Application',fg='green',font=('Arial',20),command=track_button)
    button2 = tk.Button(appWindow,text='Usage Stats',fg='lightblue',font=('Arial',20),command=usage_button)
    button3 = tk.Button(appWindow,text='App Blocker',fg='lightblue',font=('Arial',20),command=block_button)
...
def buildTrackingMenu():
    ...
    customEXEButton = Button(newFrame,text='Select App',command=selectFile)
    ...
    programStatus = Label(newFrame,textvariable=programStatusVar,fg='green')
...
appWindow = tk.Tk(className='App Usage Tracker and Blocker')
